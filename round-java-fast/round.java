        ...
        try {
            ...
            while (true) {
                ...
                String[] words1 = line1.split(" ");// splits the string based on whitespace
                 ...
                 towns = Integer.parseInt(words1[0]);
            ...
            //main loop
            for (int mainpointer = 0; mainpointer < towns; mainpointer++) {
                
                //find max and maxpointer for every city
                for (int i = 1; i < towns; i++) {
                    if (population[(mainpointer + i) % towns] != 0) {
                        maxpointer = (mainpointer + i) % towns;
                        max = mainpointer - (mainpointer + i) % towns;
                        if (max < 0)
                            max += towns;
                        break;
                    }
                ...
                //find new sum 
                if (mainpointer != 0) {
                    sum = sum + cars - (towns) * population[mainpointer];
                }
